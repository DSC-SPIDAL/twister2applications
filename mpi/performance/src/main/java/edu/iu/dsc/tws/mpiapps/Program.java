...
public class Program {
    ...
    public void execute(String[] args) throws MPIException {
        ...
        if (rank == 0) {
            System.out.println(String.format("Collective %d with size %d and iteration %d",
                    collective, dataSize, iterations));
        }
        doRun(100000); //warmup
        doRun(iterations);
    }

    private void doRun(int iterations) throws MPIException {
        int rank = MPI.COMM_WORLD.getRank();
